...
class week_statistical(models.Model):
    ...
    def _get_name(self):
        self.env.cr.execute('SELECT id FROM week_statistical WHERE week_num=%s',
        ...
        if not report:
            if self.week_statistical_mx_ids:
                ...
                for per in new_person:
                    print(len(sataistical_mx))
                    azxy_sum = len(sataistical_mx.search([('person_id', '=', per), ('type', '=', 1), ('week_statistical_id', '=', self.ids[0])]))
                    azxy_js = len(
                        sataistical_mx.search([('person_id', '=', per), ('type', '=', 1), ('in_time', '=', True),('week_statistical_id', '=', self.ids[0])]))
                    azhd_sum = len(sataistical_mx.search([('person_id', '=', per), ('type', '=', 0),('week_statistical_id', '=', self.ids[0])]))
                    azhd_js = len(
                        sataistical_mx.search([('person_id', '=', per), ('type', '=', 0), ('in_time', '=', True),('week_statistical_id', '=', self.ids[0])]))
                    wxxy_sum = len(sataistical_mx.search([('person_id', '=', per), ('type', '=', 3),('week_statistical_id', '=', self.ids[0])]))
                    wxxy_js = len(
                        sataistical_mx.search([('person_id', '=', per), ('type', '=', 3), ('in_time', '=', True),('week_statistical_id', '=', self.ids[0])]))
                    wxhd_sum = len(sataistical_mx.search([('person_id', '=', per), ('type', '=', 2),('week_statistical_id', '=', self.ids[0])]))
                    wxhd_js = len(
                        sataistical_mx.search([('person_id', '=', per), ('type', '=', 2), ('in_time', '=', True),('week_statistical_id', '=', self.ids[0])]))
                    if azxy_sum > 0:
                    ...
                    if self.env['level.department'].search([('two_level', '=', two_level)]):
                        department_id = self.env['level.department'].search([('two_level', '=', two_level)])[
                            0].department_id.id
                    else:
                        department_id = False
                ...
                for zh_per in self.env['level.report'].search(
                        [('week_statistical_id', 'in', self.ids), ('is_pm', '=', True)]):
                    zh_sum = zh_per.azxy_pm + zh_per.azhd_pm + zh_per.wxxy_pm + zh_per.wxhd_pm
                    zh_per.write({'zh_sum': zh_sum})
                self.env.cr.execute(
                    'select id, Rank() over (order by zh_sum asc) from level_report where is_pm = TRUE and week_statistical_id = %s',
                    (self.ids))
                vids = [x for x in self.env.cr.fetchall() if x[0]]
                if vids:
                    for zh in vids:
...
class LevelToDepartment(models.Model):
    ...
    _sql_constraints = [
        ('level_uniq', 'unique (two_level)', '二级网格只能一个!'),
        ('department_uniq', 'unique (department_id)', '只能对应一个部门!')
    ]
